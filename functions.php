<?php
    ...
    function check_user(){
        if(!isset($_SESSION['user_id'])){
            die("<p style='color:#575757'>You have not logged in ,please login from <a href='login.php'>here</a></p>");
        }
    ...
    function check_file_is_shared($file_id){
        echo "Yes this file is shared with you";
    }
    ...
    function check_file_owner($file_id){
        ...
        if($db_user_id!=$user_id){
            // die("<p style='color:black'>This file doesnot belongs to the logged in user</p>");
        }
    }
    ...
    function get_parent_file_id($file_id){
        /*Get the parent file the id*/
        global $connection;

        $query="SELECT * FROM files where file_id=$file_id";
        $parent_id_query=mysqli_query($connection,$query);
        $row=mysqli_fetch_assoc($parent_id_query);
